<div id="formCreateContainer" class="p-3 mb-4 rounded shadow-sm" style="display:none; background-color:#EBF5FF; border:1px solid #B3D9FF;">
    <h5 class="fw-bold mb-3" style="color:#0158a4;">Tambah Data Booster</h5>
    <hr class="my-3">

    <form id="createBoosterForm" class="row g-3">
        @csrf

        <div class="col-md-10">
            <label class="form-label fw-bold">Nama Booster</label>
            <input type="text" class="form-control" id="create_namaBooster" name="nama" required placeholder="Contoh: Booster">
        </div>
...
$('#create_cancelBtn').click(function(){
    $('#createBoosterForm')[0].reset();
    $('#formCreateContainer').slideUp();
});

$('#createBoosterForm').submit(function(e){
    e.preventDefault();
    const fd={
        nama:$('#create_namaBooster').val(),
        _token:'{{ csrf_token() }}'
    };
