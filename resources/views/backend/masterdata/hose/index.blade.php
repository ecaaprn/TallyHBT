...
<div class="card shadow rounded">
    <div class="card-header bg-white d-flex flex-wrap justify-content-between align-items-center" style="border-bottom: none !important;">
        <h4 class="mb-2 mb-md-0 fw-bold text-uppercase" style="color: #000000;">DATA HOSE</h4>
        <div class="d-flex align-items-center gap-2" style="justify-content: flex-end; flex-wrap: nowrap;">
            <input type="text" id="searchInput" class="form-control" placeholder="Cari hose" style="border-radius: 8px; width: 300px; min-width: 180px;">
    ...
    <div class="card-body">
        <div id="formContainerWrapper">
            @include('backend.masterdata.hose.create')
            @include('backend.masterdata.hose.edit')
        </div>
            ...
            <table class="table align-middle table-hover" style="min-width: 750px; text-align: center; font-size: 0.95rem;">
                <thead>
                    <tr>
                        ...
                        <th style="background-color: #0158a4; color: #fff; white-space: nowrap; padding: 12px 8px;">Nama Hose</th>
...
<script>
const baseUrl = "{{ url('admin/masterdata/hose') }}";
...
function renderPaginationNumeric(links){
    ...
    filteredLinks.forEach(l => {
        ...
        if(l.active){
            ...
            btn.click(() => fetchHoseData(currentSearch, page));
        } else {
...
function fetchHoseData(q='', page=1){
    currentSearch = q;

    $.get(`${baseUrl}?search_hose=${encodeURIComponent(q)}&page=${page}&ajax=1`, function(res){
        const p = res.hoses;
        const data = p.data;
        const tbody = $('#hoseTableBody');
        tbody.empty();
        ...
        data.forEach((h,i)=>{
            tbody.append(`
                <tr>
                    ...
                    <td style="white-space: nowrap;">${h.nama}</td>
                    <td style="white-space: nowrap;">${formatDateTime(h.created_at)}</td>
                    <td style="white-space: nowrap;">${formatDateTime(h.updated_at)}</td>
                    <td style="white-space: nowrap;">
                        <button class="btn btn-sm editHoseBtn"
                                data-id="${h.id}"
                                data-nama="${h.nama}">
        ...
        $('#hoseTableBody').html(`<tr><td colspan="5" class="text-center text-danger py-3">Gagal memuat data.</td></tr>`);
    });
}
...
$(document).on('click', '.editHoseBtn', function() {
    $('#formCreateContainer').stop(true, true).slideUp(300);

    const hoseName = $(this).data('nama');
    $('#edit_namaHose').val(hoseName);
...
$('#searchInput').on('input',function(){
    ...
    searchTimer = setTimeout(()=>fetchHoseData($(this).val()),400);
});
