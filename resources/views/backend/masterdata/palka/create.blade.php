<div id="formCreateContainer" class="p-3 mb-4 rounded shadow-sm" style="display:none; background-color:#EBF5FF; border:1px solid #B3D9FF;">
    <h5 class="fw-bold mb-3" style="color:#0158a4;">Tambah Data Palka</h5>
    <hr class="my-3">

    <form id="createPalkaForm" class="row g-3">
        @csrf

        <div class="col-md-10">
            <label class="form-label fw-bold">Nama Palka</label>
            <input type="text" class="form-control" id="create_namaPalka" name="nama" required placeholder="Contoh: Palka-1">
        </div>
...
$('#createPalkaForm').submit(function(e){
    e.preventDefault();
    const fd={
        nama:$('#create_namaPalka').val(),
        _token:'{{ csrf_token() }}'
    };
